                                                   ...
                                                   lit_utf8_size_t string_size) /**< utf-8 string size */
{
  ...
  else
  {
    ...
    else
    {
      ...
      long_string_desc_p->long_utf8_string_length = 0;
                                    ...
                                    jmem_cpointer_t prop_name_cp) /**< property name compressed pointer */
{
  switch (ECMA_PROPERTY_GET_NAME_TYPE (property))
  {
    case ECMA_DIRECT_STRING_PTR:
    {
      ...
      {
        lit_utf8_size_t size;
        if (prop_name_p->refs_and_container & ECMA_STRING_CONTAINER_HEAP_LONG_UTF8_STRING)
        {
          size = prop_name_p->u.long_utf8_string_size;
        }
        else
        {
          size = prop_name_p->u.utf8_string.size;
        }
        prop_name_p->hash = lit_utf8_string_calc_hash ((lit_utf8_byte_t *) (prop_name_p + 1), size);
      }
                           ...
                           ecma_string_t *string2_p) /**< ecma-string */
{
  ...
  {
      lit_utf8_size_t size;
      if (string1_p->refs_and_container & ECMA_STRING_CONTAINER_HEAP_LONG_UTF8_STRING)
      {
        size = string1_p->u.long_utf8_string_size;
      }
      else
      {
        size = string1_p->u.utf8_string.size;
      }
...
ecma_string_get_length (ecma_string_t *string_p) /**< ecma-string */
{
  ...
  switch (ECMA_STRING_GET_CONTAINER (string_p))
  {
    case ECMA_STRING_CONTAINER_HEAP_UTF8_STRING:
    {
      if (string_p->u.utf8_string.size && !string_p->u.utf8_string.length)
      {
    ...
    {
      ecma_long_string_t *long_string_p = (ecma_long_string_t *) string_p;
      if (string_p->u.utf8_string.size && !long_string_p->long_utf8_string_length)
      {
          ECMA_STRING_TO_UTF8_STRING (string_p, utf8_str_p, utf8_str_size);
 ...
 */
lit_utf8_size_t
ecma_string_get_utf8_size (ecma_string_t *string_p) /**< ecma-string */
{
  ecma_length_t length = ecma_string_get_ascii_size (string_p);
...
void
ecma_string_construct_buffer_initialize (ecma_string_construct_buffer_t *buf_p, ecma_string_t *init_str_p)
{
...
void
ecma_string_construct_buffer_append (ecma_string_construct_buffer_t *strbuf_p, ecma_string_t *str_p)
{
...
ecma_string_t *
ecma_string_construct_buffer_finalize (ecma_string_construct_buffer_t *strbuf_p)
{
...
void
ecma_string_construct_buffer_destroy (ecma_string_construct_buffer_t *strbuf_p)
{
  jmem_heap_free_block (strbuf_p->buffer_p, strbuf_p->size);
